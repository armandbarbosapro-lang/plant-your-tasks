...
const useAmbientSound = (isNight: boolean) => {
  ...
  const getCtx = useCallback(async () => {
    ...
    // ✅ Fix iPhone : resume après interaction utilisateur
  ...
  const playNote = useCallback(async (freq: number, startTime: number, duration: number, volume: number, type: OscillatorType = "square") => {
    const { ctx, gain } = await getCtx();
  ...
  const playDayLoop = useCallback(async () => {
    ...
    const bassNotes = [
      { note: 48, time: 0, dur: beat * 6 },
      { note: 55, time: beat * 6.5, dur: beat * 5 },
      { note: 53, time: beat * 12, dur: beat * 5 },
      { note: 48, time: beat * 17, dur: beat * 4 },
      { note: 53, time: beat * 21.5, dur: beat * 3 },
      { note: 48, time: beat * 24, dur: beat * 4 },
    ];
    ...
    melodyNotes.forEach(n => playNote(noteFreq(n.note), now + n.time, n.dur, 0.18, "square"));
    bassNotes.forEach(n => playNote(noteFreq(n.note), now + n.time, n.dur, 0.12, "triangle"));
    arps.forEach(n => playNote(noteFreq(n.note), now + n.time, beat * 0.4, 0.06, "triangle"));

    const loopDuration = beat * 28;
    loopRef.current = window.setTimeout(() => playDayLoop(), loopDuration * 1000);
  }, [getCtx, playNote, noteFreq]);

  const playCrickets = useCallback(async (startTime: number, duration: number) => {
    const { ctx, gain } = await getCtx();
  ...
  const playOwlHoot = useCallback(async (startTime: number) => {
    const { ctx, gain } = await getCtx();
  ...
  const playWindGust = useCallback(async (startTime: number) => {
    const { ctx, gain } = await getCtx();
  ...
  const playNightLoop = useCallback(async () => {
    const { ctx } = await getCtx();
    const now = ctx.currentTime;
    const loopDuration = 12;
    playCrickets(now, loopDuration);
    const r = Math.random();
    if (r < 0.3) playOwlHoot(now + 2 + Math.random() * 4);
    else if (r < 0.55) playWindGust(now + 1 + Math.random() * 3);
    if (Math.random() < 0.15) playOwlHoot(now + 8 + Math.random() * 2);
  ...
  useEffect(() => {
    ...
    // ✅ Fix iPhone : délai court pour laisser le temps au resume()
    const startTimeout = setTimeout(() => {
      if (isNight) playNightLoop();
      else playDayLoop();
    }, 300);
  ...
  // ✅ Fix iPhone : toggleMute resume le contexte audio au moment du clic
